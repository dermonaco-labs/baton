...
| C4 | Review | Code review is `baton-review` → `ce-review`, with intent taken from spec/plan/tasks (and not from PR text alone). baton-review always calls `ce-review mode:headless` and normalizes the findings into Baton's `review.json`; ce-review's interactive/autofix modes (and their `todo-create` side effects) are not part of the relay. Artifact consistency before code is `speckit-analyze`. Requirements quality is `speckit-checklist`. `document-review` is optional (the `docs-review` pack) and advisory. |
