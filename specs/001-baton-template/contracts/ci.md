...
All workflows use actions pinned by full commit SHA (with a `# vX.Y.Z` comment), `permissions: {}` at the top level
...
|---|---|---|---|---|
| `ci.yml` | `pull_request`, `push` to main | `lint` (ubuntu, 5 min), `test` (ubuntu, 5 min) | `contents: read` | fast, meaningful checks |
| `smoke.yml` | `pull_request`, `push` to main, `workflow_dispatch` | `smoke` matrix {ubuntu-latest, windows-latest} (10 min) | `contents: read` | e2e: instantiate the template + overlay the fixtures |
| `upstream-watch.yml` | `schedule` (weekly, Mon 06:00 UTC), `workflow_dispatch` | `watch` (ubuntu, 5 min) | `contents: read`, `issues: write` | one tracking issue for new upstream releases/commits |
| `release.yml` | `push` tags `v*.*.*`, `workflow_dispatch` (`dry_run: true`: builds and verifies, publishes nothing) | `release` (ubuntu, 10 min) | `contents: write`, `id-token: write`, `attestations: write` | build + publish assets |
| `template-cleanup.yml` | `push` to the default branch | `cleanup` (ubuntu, 5 min), guarded by `if: github.repository != 'dermonaco-labs/baton'` plus a marker file check | `contents: write` | derived-repo cleanup (US1) |
| `copilot-setup-steps.yml` | `workflow_dispatch`, `push`/`pull_request` touching itself | `copilot-setup-steps` (ubuntu, 10 min) | `contents: read` | coding-agent environment: node 20, uv, pinned `specify-cli`, `baton doctor` |
...
1. **Template instantiation**:
   ...
   - Set `BATON_FORCE_CLEANUP=1` and run `node .baton/bin/baton.mjs adopt`.
   - Assert that the dev paths are gone, `README.md` is the adopter README and `docs/baton/` exists.
   - Run `doctor --strict` and `validate` and assert exit 0.
